...
def main(tp, run):
    ...
    for i in range(ntot):
        for j in range(ntot):
            collision_rates_matrix[i][j] = representative_particles[j].num_par * 0.5 * (representative_particles[i].mass + representative_particles[j].mass) / vol
    ...
    ## keep track of num collisions
    tot_num_collisions = 0

    # save time steps of each collision
    time_steps = []
    ...
    while sim_time < tend:
        ...
        # advance time following SSA procedure
        dtime = -math.log(random_value) / totrate
        ...
        collision_rates_matrix[:, colliding_particle1] = representative_particles[colliding_particle1].num_par * 0.5 * (representative_particles[colliding_particle1].mass + np.array([p.mass for p in representative_particles])) / vol
        ...
        if sim_time > tout:
            ...
            filename = f"/Users/calebpainter/Downloads/Classes/AM207/final_project/outputs/SSA_python_output_nparticles_{ntot}_{np.round(tout,2)}_run_{run}.txt"
            print(filename)
            ...
            np.save(f'time_steps_SSA_run_{run}.npy',time_steps)
            tout += dtout
...
if __name__ == "__main__":
    ...
    # np.save('/Users/calebpainter/Downloads/Classes/AM207/final_project/outputs/SSA_runtimes2', particle_run_times)
    # np.save('/Users/calebpainter/Downloads/Classes/AM207/final_project/outputs/SSA_runtimes_errors2', particle_run_times_errors)